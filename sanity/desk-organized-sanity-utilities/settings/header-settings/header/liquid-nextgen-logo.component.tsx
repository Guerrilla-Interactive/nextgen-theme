...
/**
 * Renders the Nextgen logo where the "GEN" part is filled by the LiquidMetal shader.
 * The base logo uses only the foreground path (no primary paths).
 * The GEN mask uses the two primary paths from your SVG.
 */
export function NextgenLogoWithLiquidGEN({
  className = "relative w-[164px] h-[22px]", // 2x the original for sharper result
}: { className?: string }) {
  // The base logo WITHOUT the "GEN" (primary) shapes — only the foreground path.
  const baseLogo = (
    <svg
      viewBox="0 0 164 22"
      className="absolute inset-0 w-full h-full"
      aria-label="Nextgen base logo (foreground only)"
    >
      <path
  ...
  // The two "GEN" (orange) paths from your SVG, used as a CSS mask.
  const genMaskSvg = useMemo(() => {
    const svg = `
      <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 164 22' preserveAspectRatio='none'>
        <path d="M159.975 0.0738854V13.9498L138.198 0.0852637L138.072 0.00561523V17.1414H118.524V12.7608H134.184V8.60767H114.526V21.2945H142.069V7.47553L163.973 21.3628V0.0738854H159.975Z" fill="white"/>
        <path d="M105.606 9.19374L101.028 12.7608H106.86V17.1415H88.3514L102.759 5.91677C104.155 4.82444 105.891 4.22707 107.638 4.22707H134.184V4.14174C134.184 1.9002 132.427 0.0739746 130.269 0.0739746H107.271C104.9 0.0739746 102.551 0.887527 100.651 2.36671L83.1275 16.0207C82.1419 16.7888 81.7531 18.0745 82.1419 19.292C82.5307 20.5095 83.582 21.2946 84.8196 21.2946H110.857V8.60775H107.298C106.69 8.60775 106.094 8.81825 105.606 9.19374Z" fill="white"/>
      </svg>`
    // inline SVG as a data URL for CSS mask-image
    return `url("data:image/svg+xml;utf8,${encodeURIComponent(svg)}")`
  }, [])

  // The shader layer that’s masked by the GEN paths
  const genLiquidLayer = (
    <div
      className="absolute inset-0"
      style={{
        WebkitMaskImage: genMaskSvg,
        maskImage: genMaskSvg,
  ...
  colorBack="hsl(0, 100%, 19.019607843137255%)"
  colorBloom="hsl(12, 100%, 50%)"
  bloom={0.6}
  intensity={0.6}
  density={0.03}
  spotty={0}
