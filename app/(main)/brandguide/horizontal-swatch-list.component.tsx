...
export const HorizontalSwatchList = ({
    colors,
    onSelectColor,
    currentlySelectedColorName,
    selectedRole,
    onClearRole
}: {
    colors: HorizontalSwatchListColor[];
    onSelectColor: (colorName: string | null) => void;
    currentlySelectedColorName: string | null;
    ...
    // But don't auto-select if we're currently viewing a role.
    useEffect(() => {
        // Don't auto-select if we're viewing a role
        if (selectedRole) {
            return;
        }
    ...
    }, [colors, currentlySelectedColorName, onSelectColor, selectedRole]);

    const handleSwatchInteraction = (color: HorizontalSwatchListColor) => {
        // Clear role selection when a color is selected (color view should take precedence)
        if (selectedRole && onClearRole) {
            onClearRole();
        }
                                    ...
                                    <div className="w-full flex flex-wrap gap-1 self-start relative z-10">
                                        {Array.from(color.roles).map((role) => (
                                            <div
                                                ...
                                                style={{ fontFamily: 'var(--font-code, var(--font-mono, monospace))' }}
                                                title={role}
                                            >
                                                {role}
                                            </div>
